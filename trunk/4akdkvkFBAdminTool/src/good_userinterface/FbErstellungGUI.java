		...
		try {
			setDefaultCloseOperation(WindowConstants.DISPOSE_ON_CLOSE);
			{
					      ...
					      public void actionPerformed(ActionEvent arg0) {				    	  
					    	  if(  !jTextField_fbTitel.getText().toString().equals("")
					    	     ...
					    	     &&!jTextField_Anzahl.getText().toString().equals( "" )){    			    		  
										...
										try {
												//Endadatum zusammenstückeln 
												String[] datum = jTextField_fbEndDatum.getText().toString().split("\\p{Punct}");
												int day = Integer.valueOf(datum[0]);	//Tag hohlen
												int month = Integer.valueOf(datum[1]); //Monat hohlen
												int year = Integer.valueOf(datum[2]); //Jahr hohlen
												Date dt;//Test bemerkung "DATUM" --> 1999 und auch 99 geht bei Jahreszahl
												Date dt2 = new Date();//heutiges Datum für prüfung benötigt
												SimpleDateFormat sdf = new SimpleDateFormat("dd.MM.yyyy");	
												dt = sdf.parse(day+"."+month+"."+year );
												sdf.format(dt); 	// ins Format dd.MM.yyyy bringen
												sdf.format(dt2);	// ins Format dd.MM.yyyy bringen
												//Prüfen ob Datum gültig ist d.h. grösser als Heute
											    
												int check =dt2.compareTo(dt);
												/* int compareTo(another Date)
												 * Vergleicht zwei Datum-Objekte und gibt 0 zurück, 
												 * falls beide die gleiche Zeit repräsentieren. 
												 * Der Rückgabewert ist kleiner 0, falls das Datum des aufrufenden Exemplars
												 * vor dem Datum von anotherDate ist, sonst größer 0.
												 */
												
												if(check ==0 || check >0){
													throw new WrongDateException("Datum ist ungültig.");
												}
										    	//Enddatum setzten
												...
												JOptionPane.showMessageDialog(jPanel_mainwindow,
				  			                              "Ungütliges Datum. Vergewissern Sie sich das das Enddatum grösser als das heutige ist.",
				  			                              "Error",
				  			                              JOptionPane.ERROR_MESSAGE) ;
											}
				...
				}
				{
					...
					jTextPane1.setText("Geben Sie hier die Anzahl der zu befragenden Personen ein, diese ist wichtig für die TAN-Generierung!");
				...
				}
				{
					...
					//jLabel_logo.setIcon(new ImageIcon("logo2//logo.jpg"));
